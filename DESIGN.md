...
# Hyperporter application notes

This file is the Shopify DESIGN.md from
[VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md),
adopted for this site in September 2026 at the founder's instruction. The
sections above are the system as published and are not edited. What follows
is how it lands on this site and where it is deliberately departed from.

## How it maps

The site was already built in bands — a dark band, then a light one, all the
way down — which is the two-track structure this system describes, so the
composition survived the change and only the palette, the type and the
component skins moved.

| This site | Track |
|---|---|
| Page heroes, the product band, the closing band, the footer | Cinematic: `{colors.canvas-night}`, full-bleed photography, thin display, white-stroked pill |
| Feature bands, points, solutions rows, pricing, security, about | Transactional: `{colors.canvas-light}` / `{colors.canvas-cream}`, filled black pill, aloe on the one featured tier |

- Display is **Inter at weight 300**, the open substitute the system names for
  Neue Haas Grotesk Display. The thin cut is the brand here: nothing in a
  display role may be set at 400 or above.
- The hero headline carries the +2.4px tracking the system specifies at 96px.
- `font-feature-settings: "ss03"` is set at the root, per the system.
- Buttons are pills everywhere, 12px/24px, Inter 16/420 — on both tracks and
  in both polarities. There is no rounded-rectangle button on the site.
- Aloe appears exactly twice, both on the light track: the featured pricing
  tier and its pill. Pistachio is available as a band fill and is unused so far.
- The warm off-white ground, the warm greys and the moss accent that this site
  used before are gone. The system admits no third canvas.

## Deviations, and why

**Amber (`#8A5409` on light, `#D79A3E` on dark) is kept.** The system has no
equivalent and would have it removed as an off-palette colour. On this site
amber is not decoration: it marks a step a person has to take, and the
product's whole claim is that the automation stops where judgement starts.
Removing it would take a meaning out of the interface that the copy depends
on. It appears only on human/manual states — never on a heading, a category
or a hover.

**Two semantic marks survive inside the product mock-ups** — a red for a late
or failed state and the red of a PDF file mark. Both are inside drawn
interface, where they carry meaning rather than brand.

**The site's own structural rules still bind**, and no design system
overrides them:
- Horizon is a network agencies search, not software a supplier signs into. Never draw a supplier inside it.
- Atlas answers only from the account's own Vault.
- SOC 2 and GDPR are targeted post-pilot, not held. Never soften this.
- The Autopilot before/after figures are pilot targets, and the sentence saying so stays beside the table.
- Portal is the platform; Autopilot is the automation on top of it.

## Known gaps

- The 137 destination pages are not in this system and remain `noindex`.
- Three About bios are `Name pending` placeholders, carried visibly.
- The public country figure says "130+" while the dataset holds 137. Unresolved.
- Photography is still unlicensed; the cinematic track leans on it heavily, so this matters more under this system than it did before.
